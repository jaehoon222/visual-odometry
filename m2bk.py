...
def visualize_trajectory(trajectory):
    ...
    if len(trajectory.shape) != 2 or trajectory.shape[0] != 3:
        print("Error: Trajectory must be 3xN array")
        return
    ...
    # 메인 궤적 플롯 (ZX)
    traj_main_plt.set_title("Trajectory (Z, X)")
    traj_main_plt.plot(locZ, locX, ".-", label="Trajectory")
    traj_main_plt.plot([locZ[0], locZ[-1]], [locX[0], locX[-1]], "--", label="Reference")
    traj_main_plt.scatter([0], [0], c="red", label="Start")
    traj_main_plt.set_xlabel("Z")
    traj_main_plt.legend()
    
    # ZY 플롯
    ZY_plt.plot(locZ, locY, ".-")
    ZY_plt.plot([locZ[0], locZ[-1]], [locY[0], locY[-1]], "--")
    ZY_plt.scatter([0], [0], c="red")
    
    # YX 플롯
    YX_plt.plot(locY, locX, ".-")
    YX_plt.plot([locY[0], locY[-1]], [locX[0], locX[-1]], "--")
    YX_plt.scatter([0], [0], c="red")
    
    # 3D 플롯
    D3_plt.plot3D(locX, locZ, locY)
    D3_plt.scatter(0, 0, 0, c="red")
    D3_plt.set_xlabel("X")
    D3_plt.set_ylabel("Z")
    D3_plt.set_zlabel("Y")
